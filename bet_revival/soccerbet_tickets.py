"""
Soccerbet Ticket Resolver
Fetches betslip details and match results from Soccerbet's API.
No login required - UUID from barcode on betslip.

Usage:
    # Single ticket (UUID from QR/barcode)
    python soccerbet_tickets.py --ticket 16694b30-64ec-4572-a940-8d76e9f3536a

    # Multiple tickets
    python soccerbet_tickets.py --tickets 16694b30-64ec-4572-a940-8d76e9f3536a <uuid2>

    # From file (one UUID per line)
    python soccerbet_tickets.py --file tickets.txt

    # Save raw JSON output
    python soccerbet_tickets.py --ticket 16694b30-64ec-4572-a940-8d76e9f3536a --output results.json --raw

Ticket status codes:
    ITicket.status             : 1 = WON, -1 = LOST, 0 = PENDING
    ticketStatusTipType.status : 1 = hit, -1 = missed, 0 = pending
"""
...
TICKET_API = "https://www.soccerbet.rs/ibet/profile/getTicketByUuid"
...
async def fetch_ticket(session: aiohttp.ClientSession, uuid: str) -> dict:
    """Fetch a single ticket's details from Soccerbet API."""
...
async def main():
    parser = argparse.ArgumentParser(
        description="Fetch Soccerbet betslip ticket details (no login required)"
    )
    ...
    if args.raw:
        ...
        print(f"\n{'=' * 85}")
        print(f"  SOCCERBET TICKET REPORT  |  {datetime.now().strftime('%Y-%m-%d %H:%M')}")
        print(f"{'=' * 85}")
