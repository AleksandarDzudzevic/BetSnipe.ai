...
async def get_table_tennis_leagues(session):
...
async def get_soccerbet_sports():
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            leagues = await get_table_tennis_leagues(session)
            if not leagues:
                return
                            ...
                            for i in range(0, len(matches), batch_size):
                                ...
                                for match, match_data in zip(batch, match_details):
                                    ...
                                    if home_team and away_team:
                                        # Get winner odds
                                        home_win = match["betMap"].get("1", {}).get("NULL", {}).get("ov", "N/A")
                                        away_win = match["betMap"].get("3", {}).get("NULL", {}).get("ov", "N/A")
