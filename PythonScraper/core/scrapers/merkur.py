...
Scrapes odds from Merkur Serbia API (same platform as MaxBet).
Uses flat odds dict {code: value} + params dict {param_key: margin_value}.
Code mappings identical to MaxBet (verified via ttg_lang endpoint).
"""
...
FOOTBALL_3WAY = {
    ...
    19: ('29', '30', '31'),      # Half with More Goals
}
...
FOOTBALL_PARAM_TOTALS = {
    48: [('homeOverUnder', '355', '356')],
    49: [('awayOverUnder', '357', '358')],
    51: [('homeOverUnderFirstHalf', '371', '372')],
    52: [('awayOverUnderFirstHalf', '373', '374')],
}
...
BASKETBALL_PARAM_HANDICAPS = {
    ...
    50: [('handicapFirstHalf', '50460', '50461')],
}
...
BASKETBALL_PARAM_TOTALS = {
    ...
    6: [('overUnderFirstHalf', '50446', '50447')],
    48: [('homeOverUnderOvertime', '50462', '50463')],
    49: [('awayOverUnderOvertime', '50464', '50465')],
    51: [('homeOverUnderFirstHalf', '50466', '50467')],
    52: [('awayOverUnderFirstHalf', '50468', '50469')],
}
...
TENNIS_PARAM_TOTALS = {
    5: [('overUnderGames', '254', '256')],
}

TENNIS_PARAM_HANDICAPS = {
    56: [('hd2', '251', '253')],
    58: [('handicapGames', '50538', '50539')],
}
...
TENNIS_3WAY = {
    63: ('51061', '51062', '51063'),
}

TENNIS_SELECTIONS = {
    65: {'50544': '2:0', '50545': '0:2', '50548': '2:1', '50549': '1:2'},
    64: {'50540': '1/1', '50541': '1/2', '50542': '2/1', '50543': '2/2'},
    66: {'51198': 'T6', '51199': '7-8', '51200': '9-12', '51201': 'T13'},
    67: {'51202': 'T6', '51203': '7-8', '51204': '9-12', '51205': 'T13'},
}
...
HOCKEY_PARAM_TOTALS = {
    ...
    6: [('overUnderFirstPeriod', '50504', '50505')],
    48: [('homeOverUnder', '355', '356')],
    49: [('awayOverUnder', '357', '358')],
}

HOCKEY_PARAM_HANDICAPS = {9: [('hd2', '201', '203')]}
...
class MerkurScraper(BaseScraper):
    ...
    Same platform as MaxBet - uses flat odds dict (code→value) + params dict.
    Code mappings identical to MaxBet (verified via ttg_lang endpoint).
    """
    ...
    @staticmethod
    def _parse_3way_markets(odds, odds_list, mapping):
    ...
    @staticmethod
    def _parse_2way_markets(odds, odds_list, mapping):
    ...
    @staticmethod
    def _parse_fixed_totals(odds, odds_list, mapping):
    ...
    @staticmethod
    def _parse_param_totals(odds, params, odds_list, mapping):
    ...
    @staticmethod
    def _parse_param_handicaps_3way(odds, params, odds_list, mapping):
        """3-way European HC: raw API sign.  Positive = home receives goals."""
        for bt, entries in mapping.items():
    ...
    @staticmethod
    def _parse_param_handicaps_2way(odds, params, odds_list, mapping):
        """2-way Asian HC: sign negated so positive = home advantage."""
        for bt, entries in mapping.items():
    ...
    @staticmethod
    def _parse_selections(odds, odds_list, mapping):
    ...
    # ========================================================================
    # Network methods (unchanged from original)
    # ========================================================================

    async def fetch_leagues(self, sport_id: int) -> List[Tuple[int, str]]:
        sport_code = INTERNAL_TO_MERKUR.get(sport_id)
        if not sport_code:
            return []
    ...
    async def _process_league(
        self, sport_id: int, league_id: int, league_name: str
    ) -> List[ScrapedMatch]:
        ...
        try:
            ...
            for i in range(0, len(league_matches), batch_size):
                ...
                tasks = [self.fetch_match_details(m["id"]) for m in batch]
