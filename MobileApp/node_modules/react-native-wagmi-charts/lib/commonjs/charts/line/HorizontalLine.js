...
function LineChartHorizontalLine({
  ...
  const { width, parsedPath, height, gutter } = _react.default.useContext(
    _Chart.LineChartDimensionsContext
  );
  const { yDomain } = (0, _useLineChart.useLineChart)();
  const y = (0, _reactNativeReanimated.useDerivedValue)(() => {
    if (typeof at === 'number' || at.index != null) {
      ...
      const yForX =
        (0, _reactNativeRedash.getYForX)(
          parsedPath,
          (0, _getXPositionForCurve.getXPositionForCurve)(parsedPath, index)
        ) || 0;
