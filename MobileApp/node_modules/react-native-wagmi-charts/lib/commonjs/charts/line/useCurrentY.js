...
function useCurrentY() {
  const { path, width } = (0, _react.useContext)(
    _Chart.LineChartDimensionsContext
  );
  const { currentX } = (0, _react.useContext)(_Context.LineChartContext);
  const parsedPath = (0, _react.useMemo)(
    () => (path ? (0, _reactNativeRedash.parse)(path) : undefined),
    [path]
  );
